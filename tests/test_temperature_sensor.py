...
def device():
    ...
    with patch("temperature_sensor.random.uniform", return_value=0.0):
        with DeviceTestContext(TemperatureSensor, process=False) as proxy:
            yield proxy
...
def device_forced_low():
    ...
    with patch("temperature_sensor.random.uniform", side_effect=[-5.0] + [0.0] * 1000):
        with DeviceTestContext(TemperatureSensor, process=False) as proxy:
            yield proxy
